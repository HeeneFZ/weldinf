...
def non_max_suppression_face(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=()):
    ...
    """
    #修改此处nc = prediction.shape[2] - 15  # number of classes
    nc = prediction.shape[2] - 11  # number of classes
    ...
    t = time.time()
    # 修改此处output = [torch.zeros((0, 16), device=prediction.device)] * prediction.shape[0]  x的构成为(xyxy, conf, landmarks, cls），所以源代码分别是（4+1+10+1=16）,而改后的代码为（4+1+6+3）=14
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            #修改此处v = torch.zeros((len(l), nc + 15), device=x.device)
            v = torch.zeros((len(l), nc + 11), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
            #修改此处v[range(len(l)), l[:, 0].long() + 15] = 1.0
            v[range(len(l)), l[:, 0].long() + 11] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        # Compute conf
        #修改此处x[:, 15:] *= x[:, 4:5]
        x[:, 11:] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            # 修改此处i, j = (x[:, 15:] > conf_thres).nonzero(as_tuple=False).T
            i, j = (x[:, 11:] > conf_thres).nonzero(as_tuple=False).T
            #修改此处x = torch.cat((box[i], x[i, j + 15, None], x[:, 5:15] ,j[:, None].float()), 1)
            x = torch.cat((box[i], x[i, j + 11, None], x[i, 5:11] ,j[:, None].float()), 1)
        else:  # best class only
            # 修改此处conf, j = x[:, 15:].max(1, keepdim=True)
            conf, j = x[:, 11:].max(1, keepdim=True)
            # 修改此处x = torch.cat((box, conf, x[:, 5:15], j.float()), 1)[conf.view(-1) > conf_thres]
            x = torch.cat((box, conf, x[:, 5:11], j.float()), 1)[conf.view(-1) > conf_thres]
        ...
        # Batched NMS
        # 修改此处c = x[:, 15:16] * (0 if agnostic else max_wh)  # classes
        c = x[:, 11:12] * (0 if agnostic else max_wh)  # classes
