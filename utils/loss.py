    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                #修改此处t = torch.full_like(ps[:, 15:], cn, device=device)
                t = torch.full_like(ps[:, 11:], cn, device=device)  # targets
                t[range(n), tcls[i]] = cp
                #修改此处lcls += BCEcls(ps[:, 15:], t)
                lcls += BCEcls(ps[:, 11:], t)  # BCE
            ...
            #修改此处plandmarks = ps[:,5:15]
            plandmarks = ps[:,5:11]

            plandmarks[:, 0:2] = plandmarks[:, 0:2] * anchors[i]    #缩放为原来坐标
...
def build_targets(p, targets, model):
    ...
    # 修改此处gain = torch.ones(17, device=targets.device)
    gain = torch.ones(13, device=targets.device)
    ...
    for i in range(det.nl):
        ...
        #landmarks 10
        #修改此处gain[6:16] = torch.tensor(p[i].shape)[[3, 2, 3, 2, 3, 2, 3, 2, 3, 2]]  # xyxy gain
        gain[6:12] = torch.tensor(p[i].shape)[[3, 2, 3, 2, 3, 2]]  # xyxy gain
